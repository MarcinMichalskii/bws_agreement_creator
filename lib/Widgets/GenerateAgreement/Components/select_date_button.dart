...
class SelectDateTimeButton extends HookWidget {
  const SelectDateTimeButton(
      {super.key,
      required this.exactDate,
      required this.headerText,
      required this.onDateAndTimeSelected});
  final ExactDate exactDate;
  final String headerText;
  final void Function(ExactDate) onDateAndTimeSelected;

  @override
  Widget build(BuildContext context) {
    final onShowTimePicker = useCallback((DateTime date) async {
      final value =
          await showTimePicker(context: context, initialTime: exactDate.time);
      if (value != null) onDateAndTimeSelected(ExactDate(date, value));
    });
    final onPress = useCallback(() async {
      final value = await showDatePicker(
          context: context,
          initialDate: exactDate.date,
          firstDate: DateTime.now().add(const Duration(days: -365 * 100)),
          lastDate: DateTime.now().add(const Duration(days: 365 * 100)));

      if (value != null) {
        onShowTimePicker(value);
      }
    }, [exactDate, onDateAndTimeSelected, onShowTimePicker]);
    return FormButtonUI(
        title:
            '${DateFormat('dd.MM.yyyy').format(exactDate.date)} ${exactDate.time.format(context)}',
